...
ulistfd([],_,_,_).
ulistfd([H|T],N,[Nleft|Nlr], [Nright|Nrr]):-
    fd_domain(H, 1, N),
    fd_all_different(H),
    fd_labeling(H),
    fit_row(H, [0], 0, Count), Nleft = Count,
    reverse(H,Hrev),
    fit_row(Hrev, [0], 0, Count1), Nright = Count1,
    ulistfd(T,N,Nlr,Nrr).

ulistfd_bottom([],_,_,_).
ulistfd_bottom([H|T],N,[Ntop|Ntr], [Nbottom|Nbr]):-
    fd_domain(H, 1, N),
    fd_all_different(H),
    fd_labeling(H),    
    fit_row(H, [0], 0, Count), Ntop = Count,
    reverse(H,Hrev),
    fit_row(Hrev, [0], 0, Count1), Nbottom = Count1,
    ulistfd_bottom(T,N,Ntr,Nbr). 
...
tower(N, T, C):-
   ...
   length(T,N),
   make_row(T,N),
   ulistfd(T,N,Left,Right),
   %maplist(fd_labeling, T),
   transpose(T, T_tran),
   %make sure the new list of lists are unique and generate them row by row
   ulistfd_bottom(T_tran,N,Top,Bottom).
   %(TA slid) to generate solution to the matrix before passing down
...
time_plain(X):-
	...
	plain_tower(4, _, counts([3,2,1,3],[1,2,4,2],[3,2,2,1],[2,3,1,2])),
	statistics(cpu_time, [End|_]),
	X is End - Start.

time_fd(X):-
    statistics(cpu_time, [Start|_]),
	tower(4, _, counts([3,2,1,3],[1,2,4,2],[3,2,2,1],[2,3,1,2])),
	statistics(cpu_time, [End|_]),
	X is End - Start.    
